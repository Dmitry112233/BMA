...
<div id="allExp" class="col-lg-9 col-md-9 col-sm-9 col-xs-12">
	<br>
	<c:if test="${expressList.size() > 0}">
		...
		<c:forEach var="express" items="${expressList}">
			...
			<div class="dataSourse">
				<p>
					Источник: <a rel="nofollow" target="_blank"
						href="${express.source}">vprognoze.ru</a>
				</p>
			</div>
			<div>
				<p>
					Итоговый коэффициент: <b> <fmt:formatNumber type="number"
							maxFractionDigits="2" minFractionDigits="2"
							value="${express.resultCoeff}" />
					</b>
				</p>
			</div>
			<a class="toEventDescription"
				href="${contextPath}/express_description_${express.expressID}_details_${currentPage}">Описание
				экспресса</a>
	...
	<c:if test="${expressList.size() == 0}">
		<p class="DataMissed">
			Данные отсутствуют по непонятной причине.<br>Приносим свои
			извинения за неудобства.<br>Мы работаем над устранением
			проблемы.<br>Попробуйте обновить страницу через несколько минут.
		</p>
	</c:if>
			...
			<c:forEach var="button" items="${pageMass}">
				<c:if test="${button + 1 == currentPage}">
					<li class="page-item active"><a class="page-link"
						href="${contextPath}/expresses_list_${button * 20}">${button + 1}</a></li>
				</c:if>
				<c:if test="${button + 1 != currentPage}">
